...
new class extends Component {
    ...
    {
        $this->validate([
            "uploads.$type" => 'required|file|mimes:pdf|max:5120',
        ]);
...
<div class="space-y-8">
    ...
    {{-- Progress --}}
    @php
        $approved = collect($this->requiredTypes)->filter(fn($t) => data_get($this->docs, "$t.status") === 'approved')->count();
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-zinc-900" data-tour="eligibility-progress">
        ...
        <div class="h-2 rounded-full bg-gray-100 dark:bg-zinc-800 overflow-hidden">
            <div class="h-full bg-indigo-600 dark:bg-indigo-500 transition-all duration-500" style="width: {{ $pct }}%">
            </div>
        </div>
        <div class="mt-4 flex justify-end">
            <div
                class="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:ring-amber-800/50">
        ...
        @foreach($requiredTypes as $type)
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-zinc-900">
                <div class="flex items-start justify-between mb-4">
                    ...
                    <span
                        class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $map['color'] }}">
                ...
                <p class="mb-6 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    @else
                        Approved
                        {{ \Illuminate\Support\Carbon::parse($doc['reviewed_at'] ?? $doc['updated_at'] ?? now())->diffForHumans() }}
                    @endif
                </p>

                @if($doc && $doc['path'])
                    <div
                        class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3 flex items-center justify-between dark:border-zinc-700 dark:bg-zinc-800/50">
                        <div class="flex items-center gap-2 overflow-hidden">
                            <flux:icon name="document" class="size-4 text-gray-400" />
                            <span
                                class="truncate text-sm font-medium text-gray-700 dark:text-gray-300">{{ basename($doc['path']) }}</span>
                        </div>
                        <a href="{{ Storage::disk('public')->url($doc['path']) }}" target="_blank"
                            class="text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">View</a>
                    </div>
                @endif

                <div class="flex flex-col gap-3">
                    <input type="file" wire:model="uploads.{{ $type }}" accept="application/pdf"
                        class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:text-gray-400 dark:file:bg-indigo-900/30 dark:file:text-indigo-400 cursor-pointer" />
    ...
    <div
        ...
        <ul class="space-y-2 text-sm text-indigo-900/90 dark:text-indigo-200/80">
            <li class="flex items-start gap-2">
                <flux:icon name="check" class="size-4 mt-0.5" /> All documents must be in PDF format
            </li>
            <li class="flex items-start gap-2">
                <flux:icon name="check" class="size-4 mt-0.5" /> Maximum file size is 5MB per document
            </li>
            <li class="flex items-start gap-2">
                <flux:icon name="check" class="size-4 mt-0.5" /> Transcript must be an official copy
            </li>
            <li class="flex items-start gap-2">
                <flux:icon name="check" class="size-4 mt-0.5" /> Processing takes 2–3 business days after all documents
                are uploaded
            </li>
        </ul>
    </div>
