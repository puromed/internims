        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div class="flex items-center gap-4">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                    {{ $status ? ucfirst(str_replace('_', ' ', $status)) : 'All' }} logbooks
                </h3>
            ...
            @if(count($selected) > 0)
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="openRevisionModal" wire:loading.attr="disabled"
        ...
        @else
            ...
            <div class="space-y-4">
                @foreach($entries as $entry)
                    ...
                    <div
                        class="rounded-xl border border-gray-200 dark:border-gray-700 p-4 flex items-start gap-4 {{ in_array($entry->id, $selected) ? 'ring-2 ring-indigo-500 dark:ring-indigo-400' : '' }}">
                        {{-- Checkbox --}}
                        <input type="checkbox" wire:model.live="selected" value="{{ $entry->id }}"
                        ...
                        <div class="flex-1 flex items-start justify-between gap-3">
                            ...
                            <div class="flex flex-col items-end gap-2">
                                <span
                                    class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ $statusMap['class'] }}">
                                    {{ $statusMap['label'] }}
                                </span>

                                <a href="{{ route('faculty.logbooks.show', $entry) }}" wire:navigate
    ...
    @if($showRevisionModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                {{-- Backdrop --}}
                <div wire:click="closeRevisionModal"
                    class="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/80 transition-opacity"></div>

                {{-- Modal panel --}}
                <div
                    class="relative inline-block transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                    ...
                    <div class="p-6">
                        <textarea wire:model="bulkComment" rows="4"
                            ...
                            placeholder="Please add more details about your activities this week..."></textarea>
                        @error('bulkComment')
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 flex justify-end gap-3">
                        ...
                        <button wire:click="bulkRequestRevision" wire:loading.attr="disabled"
